function sort_cols(list_options, table) {
  return Object.fromEntries(
    Object.entries(list_options[table]).map(([k, v]) => {
      if (v.type == "date" || v.type == "string" || v.type == "email") {
        ...
        return [
          k.toUpperCase(),
          (array) =>
            array.sort((a, b) => (a[k] && b[k] ? a[k] - b[k] : a[k] ? -1 : 1)),
        ];
      } else if (v.type == "boolean") {
        return [
          k.toUpperCase(),
          (array) => array.sort((a, b) => (a[k] === b[k] ? 0 : a[k] ? -1 : 1)),
        ];
        ...
        return [
          ...
          (array) =>
            array.sort((a, b) => {
              let name_fields = Object.keys(list_options[v.name]).filter(
                (col) => col.includes("name")
              );
              if (name_fields.length > 0) {
                ...
                return a[k] && b[k]
                  ? a[k][name_field].localeCompare(b[k][name_field], {
                      sensitivity: "base",
                    })
                  : a[k]
                  ? -1
            ...
            array.sort((a, b) => {
              ...
              if (name_fields.length > 0) {
                ...
                return a[k].length > 0 && b[k].length > 0
                  ? a[k][0][name_field].localeCompare(b[k][0][name_field], {
                      sensitivity: "base",
                    })
                  : a[k].length > 0
                  ? -1
